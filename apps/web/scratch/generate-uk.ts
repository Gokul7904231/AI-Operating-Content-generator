...
async function run() {
  console.log("=======================================================");
  console.log("  GENERATING SHORTSFACTORY VIDEO FOR THE UK          ");
  console.log("=======================================================");
  ...
  // 2. Fetch quiz draft for the United Kingdom
  console.log("\nRequesting quiz draft for the United Kingdom (GB)...");
  ...
  const payload = {
    ...
    hashtags: ["quiz", "uk", "britain", "geoquiz", "shorts"],
    renderProfile: "AUTO",
    durationSeconds: 60,
  };
  ...
  // 4. Poll job status
  console.log("\nPolling status until UK compilation completes...");
  let status = "queued";
  let elapsed = 0;
  ...
  while (elapsed < maxWait) {
    ...
    if (status === "completed") {
      console.log("\n=======================================================");
      console.log("  🎉 UK SHORTS COMPILATION COMPLETED SUCCESSFULLY!  ");
      console.log("=======================================================");
  ...
  console.error("\n❌ Timeout reached waiting for UK video compilation.");
  process.exit(1);
}

run().catch((err) => {
  console.error("Error running UK generator:", err);
  process.exit(1);
});
