...
async function run() {
  console.log("=======================================================");
  console.log("  COMMENCING SHORTSFACTORY AUTO PROFILE E2E TEST       ");
  console.log("=======================================================");

  // 1. Check dev server health
  console.log(`Step 1: Checking dev server health on ${BASE_URL} ...`);
  let healthy = false;
  for (let attempt = 1; attempt <= 6; attempt++) {
    try {
      ...
      console.warn(`[Attempt ${attempt}/6] Health check failed: ${err.message}`);
    }
    await new Promise((r) => setTimeout(r, 2000));
  }

  if (!healthy) {
    console.error(`❌ E2E Failed: Local dev server at ${BASE_URL} is not reachable.`);
    process.exit(1);
  }
  ...
  // 3. Trigger video generation in AUTO mode
  console.log("\nStep 3: Triggering video generation pipeline in AUTO mode...");
  const payload = {
    ...
    renderProfile: "AUTO",
    durationSeconds: 60, // AUTO will resolve profile duration based on narration speech pacing quality
  };
  ...
  let status = "queued";
  let elapsed = 0;
  const maxWait = 240; // 4 minutes timeout
  ...
  while (elapsed < maxWait) {
    ...
    try {
      ...
      console.warn(`⚠️ Polling error:`, pollErr.message);
    }
  ...
  // Contract Assertion 1: Verify question counts
  ...
  if (totalQuestions === 0) {
    console.error(`❌ E2E Failed: No question scenes found.`);
    process.exit(1);
  }
  console.log(`✅ Question count contract satisfied (${totalQuestions} questions).`);
  ...
  for (const wavName of voiceFiles) {
    ...
    if (dataOffset === -1) {
      console.error(`❌ E2E Failed: data chunk not found in WAV header of ${wavName}.`);
      process.exit(1);
    }
  ...
  // Load target duration dynamically from solved Timeline JSON
  const timelinePath = path.join(tempDir, "Timeline.json");
  if (!fs.existsSync(timelinePath)) {
    console.error("❌ E2E Failed: Timeline.json not found in job assets directory.");
    process.exit(1);
  }
  const timeline = JSON.parse(fs.readFileSync(timelinePath, "utf8"));
  const targetDuration = timeline.duration;
  console.log(`\nTimeline resolved profile: ${timeline.resolvedProfile} | Resolved Duration: ${targetDuration}s`);
  ...
  // Assert target duration resolved dynamically
  ...
  if (firstFrameResult.status !== 0) {
    console.error("❌ E2E Failed: FFmpeg first frame extraction failed.");
    process.exit(1);
  }
  ...
  if (lastFrameResult.status !== 0) {
    console.error("❌ E2E Failed: FFmpeg last frame extraction failed.");
    process.exit(1);
  }
  ...
  console.log("\n=======================================================");
  console.log("  🎉 AUTO PROFILE E2E TEST COMPLETED WITH 100% PASS   ");
  console.log("=======================================================");
}
