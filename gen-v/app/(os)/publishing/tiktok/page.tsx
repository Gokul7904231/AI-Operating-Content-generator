...
import { Film, CheckCircle2, Clock, XCircle, Link2, RefreshCw } from "lucide-react";
...
export default function TiktokPublishingPage() {
  ...
  const { data, isLoading, refetch } = useQuery<{ posts: TiktokPost[] }>({
    queryKey: ["tiktok-posts"],
    queryFn: async () => {
      return {
        posts: [
          { id: "tk_001", caption: "Rate my coding setup 💻 #developer #coding #setup", status: "published", publishTime: new Date().toISOString() },
          { id: "tk_002", caption: "This CLI trick saves so much time! ⚡ #lifehacks #terminal", status: "draft" },
        ]
      };
    }
  });
  ...
  return (
    <div className="space-y-6 pb-12">
      <div className="flex items-start justify-between">
        <div>
          <h1 className="text-2xl font-bold text-zinc-100">TikTok Publishing</h1>
          <p className="text-sm text-zinc-500 mt-1">Schedule uploads to your TikTok account, manage drafts, and inspect publishing history</p>
        </div>
      ...
      <div className="rounded-xl border border-zinc-800 bg-zinc-900/60 p-5 flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h3 className="text-sm font-semibold text-zinc-150">ShortFactory Creator TikTok</h3>
      ...
      <div className="rounded-xl border border-zinc-800 overflow-hidden bg-zinc-900/40">
        <div className="px-5 py-3 border-b border-zinc-800 bg-zinc-900/50">
          <h2 className="text-sm font-semibold text-zinc-300">TikTok Queue & History</h2>
        </div>
                ...
                <div className="shrink-0 text-right text-xs">
                  {post.status === "published" ? (
                    ...
                    <span className="flex items-center gap-1.5 text-amber-400 font-medium">
                      <Clock className="w-3.5 h-3.5 animate-pulse" /> Draft
                    </span>
                  )}
